...
| `GET` | `/api/health/delivery` | админ-ключ | Недоставленные заявки и когда каналы работали |
...
| `DELIVERY_RETRY_MINUTES` | как часто досылать недоставленное и проверять каналы (0 — выключить) |
| `DELIVERY_ALERT_HOURS` | через сколько часов молчания канала слать предупреждение |
...
### Если канал доставки лёг

Заявка никогда не теряется из-за прокси или SMTP: она сохраняется в базе, а дальше
работает фоновая дослыка (`DELIVERY_RETRY_MINUTES`, по умолчанию раз в 10 минут):

- заявки, которые не ушли, отправляются заново — каждая в тот канал, где не сработала,
  так что дубля во втором канале не будет; пытается `DELIVERY_RETRY_DAYS` суток;
- каналы проверяются на связь (`getMe` для Telegram, `SMTP verify` для почты);
- если канал молчит дольше `DELIVERY_ALERT_HOURS` (по умолчанию 12 часов), приходит
  предупреждение через соседний канал: про Telegram — письмом на `MAIL_TO`, про почту —
  сообщением в Telegram. Повтор — не чаще `DELIVERY_ALERT_REPEAT_HOURS`;
- когда канал оживёт, придёт сообщение «связь восстановлена», а недоставленные заявки
  уйдут сами в ближайший проход.

Посмотреть очередь и когда каналы работали в последний раз:

```bash
curl -s -H "X-Admin-Key: <ключ>" http://127.0.0.1:4000/api/health/delivery
```

Состояние («когда канал работал», «когда слали тревогу») лежит в таблице `service_state`
